...
describe('Audit analytics safety contract', () => {
  ...
  it('keeps canonical CTA attribution while stripping identifiers and pilot query data', () => {
    ...
    expect(sanitizeEventParams('cta_click', {
      cta_intent: 'private_repo_docs',
      cta_position: 'new_audit',
      cta_destination: '/docs/quickstart?repository=private#token',
      link_text: 'Set up Mneme locally',
    })).toMatchObject({
      cta_intent: 'private_repo_docs',
      cta_destination: 'https://mnemehq.com/docs/quickstart',
    });
    expect(sanitizeEventParams('cta_click', {
