...
class AuditPersistenceService:
    ...
    async def run_and_save_audit(
        ...
        if not commit_sha and repository_url:
            commit_sha = "unknown"
        ...
        audit = await self.audits.create(
            ...
            mneme_version=settings.APP_VERSION,
    ...
    async def _run_p12_audit(
        ...
        if repository_url:
            repo_path = safe_clone_repo(repository_url)
            temp_dir = repo_path
        ...
        try:
            ...
            return build_protection_audit_response(
                ...
                repository_url=repository_url,
                commit_sha="unknown",  # Would come from git in production
                mneme_version=settings.APP_VERSION,
