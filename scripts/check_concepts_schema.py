#!/usr/bin/env python3
"""Validate every /concepts/<slug>/ page carries the full schema graph.

Cohort standard: BreadcrumbList + TechArticle + DefinedTerm + FAQPage.
This complements check_concepts.py (which validates hub/card registration);
here we assert the per-page JSON-LD graph so concept pages earn FAQ rich
results and AI-citation surface.
"""
...
COHORT_DIR = REPO_ROOT / "site" / "concepts"
...
def main() -> int:
    ...
    for slug in slugs():
        ...
        for required in ("BreadcrumbList", "TechArticle", "DefinedTerm"):
            if required not in types:
                errors.append(f"{slug}: {required} JSON-LD missing")
