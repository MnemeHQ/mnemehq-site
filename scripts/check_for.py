#!/usr/bin/env python3
"""Validate /for/<persona>/ pages carry BreadcrumbList + Article + FAQPage.

The /compare/ cohort is the gold standard (BreadcrumbList + Article + FAQPage).
Persona pages are decision-maker / commercial pages, so we hold them to the
same bar: a real Article node (not WebPage) plus an FAQPage with grounded Q&A.
"""
...
def main() -> int:
    errors = []
    for slug in slugs():
        canonical = f"{SITE_BASE}/for/{slug}/"
        ...
        if article is None:
            errors.append(f"{slug}: Article JSON-LD missing")
        else:
            if not article.get("headline", "").strip():
                errors.append(f"{slug}: Article has empty headline")
            for field in ("datePublished", "dateModified"):
                if not article.get(field, "").strip():
                    errors.append(f"{slug}: Article missing {field}")
            if article.get("url") != canonical:
                errors.append(f"{slug}: Article url is {article.get('url')!r}, expected {canonical!r}")
